...
1. 点击顶部模型名称选择模型，在“管理模型”中添加或编辑多个配置（也可从 `⋮ → 模型设置` 进入）。每项填写 HTTPS 服务根地址、模型 ID 和 API 密钥。顶部显示当前实际模型，记住上次选择，旧单模型配置自动保留。接口必须兼容
   Anthropic Messages；程序追加 `/v1/messages`。支持 x-api-key 或 Bearer。
2. 开始对话，文本以流式更新。文件变更会弹出允许 / 拒绝确认。
3. 点输入区 `＋ → 文件` 导入设备文档（单文件最多 16 MiB）。同名文件另存，不覆盖原文档。
4. 文件更改保存在应用内部工作区，选择文件可导出回设备。
5. 左上角历史图标可恢复已保存会话；顶部新会话图标保留旧会话和工作区。
6. 点 `＋` 在输入框下方展开“拍照、相册、文件”，再次点击或返回键收起。拍照调用系统相机，相册选择设备图片，最多 4 张，每张最多 16 MiB；发送前缩小并转换成
...
9. 数据处理优先使用内置 Lua：模型先读取 `LuaApi`，再调用 `RunLua` 执行短脚本，或用 Write 保存 `.lua` 后按路径执行。无需安装 Python，也无需启用 Shell。执行需确认，可访问工作区文件与文档列出的 xutils API。规则随 APK 自动加载，详细范围见 [ANDROID_TOOLS.md](ANDROID_TOOLS.md)。
...
附件与模型回归（模拟器，无真实 API 请求）：
```sh
adb shell am instrument -w -e mode compose app.codua2a.test/app.codua2a.NativeRuntimeTest
```
验证旧配置迁移、多配置加密保存、模型切换、附件展开收起及相机返回空 Intent 时的照片导入。
